...
- **Повторный вход:** если задача снова оказывается в статусе `verify` (например, из-за `revert` или `retry_from_blocked`), `runVerifier` проверяет наличие секции `## Verification` в `reviewComments`. При её наличии субагент не вызывается — стадия считается пройденной, задача переводится в `review`.

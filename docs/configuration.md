...
| `AGENT_GIT_PUBLISH_TIMEOUT_MS`                         | number  | `120000`                         | Timeout for the agent's internal publish request to the API when publishing a GitLab MR or GitHub PR. The API publish handler runs several upstream GitLab/GitHub calls sequentially (each with its own 30s timeout + retries), so a 30s client timeout aborts before the API responds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `AGENT_ACTIVITY_SILENCE_MS`                            | number  | `300000`                         | UI "working vs hung" threshold: a task in an in-progress stage is shown as hung when no activity (tool completion, subagent start, or in-flight tool start) has arrived within this window. Distinct from `AGENT_STAGE_STALE_TIMEOUT_MS`, which governs the server-side stale-stage watchdog.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `AGENT_MAX_TOOL_CALLS_PER_STAGE`                       | number  | `500`                            | Loop-detection guard: if a single stage exceeds this many tool calls, the stage is aborted and the task moves to `blocked_external` with reason `possible_loop` (manual intervention required, no auto-retry). Prevents runaway tool-call loops from burning hours/tokens.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `AGENT_LOOP_READ_ONLY_BURST`                           | number  | `20`                             | Loop-detection guard: if a stage makes this many consecutive read-only tool calls (Read/Glob/Grep/Bash `git show \| diff \| cat \| sed \| rg` etc.) with no intervening write (Edit/Write/Bash `git add \| commit \| push`), the stage is aborted and the task moves to `blocked_external` with reason `possible_loop`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `AIF_BOOTSTRAP_RUNTIME_PROFILE_ENABLED`                | boolean | `false`                          | Auto-provision a global runtime profile from env at API startup (off by default). When `true`, the API creates (or optionally updates) a global runtime profile named by `AIF_BOOTSTRAP_RUNTIME_PROFILE_NAME` and — unless `AIF_BOOTSTRAP_SET_DEFAULTS=false` — assigns it as the app-wide default for task/plan/review/chat. Idempotent: an existing profile with the same name is left untouched unless `AIF_BOOTSTRAP_FORCE_UPDATE=true`. Only the API-key env var _name_ is persisted, never the key value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `AIF_BOOTSTRAP_RUNTIME_PROFILE_NAME`                   | string  | `Bootstrap (Codex CLI)`          | Name of the auto-provisioned global runtime profile                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `AIF_BOOTSTRAP_RUNTIME_ID`                             | string  | `codex`                          | Runtime id of the auto-provisioned profile (e.g. `codex`, `claude`, `openrouter`, `opencode`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `AIF_BOOTSTRAP_PROVIDER_ID`                            | string  | `openai`                         | Provider id of the auto-provisioned profile (e.g. `openai`, `anthropic`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `AIF_BOOTSTRAP_TRANSPORT`                              | string  | `cli`                            | Transport of the auto-provisioned profile (`cli`, `sdk`, `api`, `app-server`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `AIF_BOOTSTRAP_BASE_URL`                               | string  | _(optional)_                     | Base URL for the auto-provisioned profile. Falls back to `CODEX_BASE_URL` when unset                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `AIF_BOOTSTRAP_API_KEY_ENV_VAR`                        | string  | `OPENAI_API_KEY`                 | Name of the environment variable holding the API key for the auto-provisioned profile (only the name is stored)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `AIF_BOOTSTRAP_DEFAULT_MODEL`                          | string  | _(optional)_                     | Default model for the auto-provisioned profile. Falls back to `OPENAI_MODEL` when unset                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `AIF_BOOTSTRAP_SET_DEFAULTS`                           | boolean | `true`                           | When `true`, the bootstrap also assigns the created profile as the app-wide default for task/plan/review/chat stages                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `AIF_BOOTSTRAP_FORCE_UPDATE`                           | boolean | `false`                          | When `true` and a profile with `AIF_BOOTSTRAP_RUNTIME_PROFILE_NAME` already exists, update it from env instead of skipping (env becomes the source of truth)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `AIF_GIT_BOT_NAME`                                     | string  | _(optional)_                     | Git `user.name` for commits made by the agent (bot attribution). When both `AIF_GIT_BOT_NAME` and `AIF_GIT_BOT_EMAIL` are set, the agent applies them as the global git identity at startup so subagent commits are attributed to the bot account                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `AIF_GIT_BOT_EMAIL`                                    | string  | _(optional)_                     | Git `user.email` for commits made by the agent (bot attribution). See `AIF_GIT_BOT_NAME`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `AIF_AGENT_DEFINITIONS_DIR`                            | string  | `<module-anchor>/.claude/agents` | Directory from which the agent reads scope rules (project/review) out of `.claude/agents/plan-coordinator.md`. Highest-priority override for container/absent-cwd deployments. When unset, the directory is resolved from the module location (`packages/agent/src \| dist` → `repo root/.claude/agents`), with `process.cwd()` as the last-resort fallback — cwd drifts across tests, containers, and supervisors.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `AGENT_INTERNAL_URL`                                   | string  | `http://agent:3010`              | Base URL the api uses to reach the agent over the docker network. Serves the Codex login broker (dev) and the always-on internal API (e.g. `POST /gitlab/prepare`) used to auto-prepare the local git repo on GitLab Connect/Sync now                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
`AIF_BOOTSTRAP_RUNTIME_PROFILE_ENABLED=true` auto-provisions a global runtime profile at
API startup (gitlab-demo.md steps 3.1 + 3.3). This removes the manual
`POST /runtime-profiles` + `PUT /settings/runtime-defaults` curl steps from a demo/CI setup:

```dotenv
AIF_BOOTSTRAP_RUNTIME_PROFILE_ENABLED=true
AIF_BOOTSTRAP_RUNTIME_PROFILE_NAME=router.ai (Codex CLI)
...
- **Off by default** — existing installs (including Codex OAuth `codex login`) are untouched.
- **Idempotent** — a second boot with the same profile name skips creation; `AIF_BOOTSTRAP_FORCE_UPDATE=true` switches to upsert-from-env.
- **No secrets persisted** — the profile stores the API-key env var _name_, never the value.
- Boot logs include a `profile-bootstrap` entry with the resulting action (`created`/`updated`/`skipped`).

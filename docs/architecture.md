...
- **Loop detection:** Each stage is guarded against runaway tool-call loops — a hard cap (`AGENT_MAX_TOOL_CALLS_PER_STAGE`, default 500) and a consecutive read-only burst with no intervening write (`AGENT_LOOP_READ_ONLY_BURST`, default 20). When tripped, the stage aborts and the task moves immediately to `blocked_external` with reason `possible_loop` (no auto-retry; a human decides next). Detection runs from tool-completion events, so it works for all transports including CLI.

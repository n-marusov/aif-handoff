...
2. **Контракт как независимый oracle** — OpenAPI/схемы WS/типы проверяют форму и семантику сообщений.

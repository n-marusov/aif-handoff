...
> **Методологическая база:** `README.md` (пирамида тестов, матрица качества, измеримость, shift-left), `gates.md` (FG-гейты), функции HF1–HF12 (`vision.md`), UC (`use-cases/`), BR (`business-rules/`). Для unit-уровня применяется единый harness-каркас: `Constrain` (изоляция и лимиты), `Inform` (trace и критерии), `Verify` (детерминированные assertions), `Correct` (retry/fallback/escalation по политике). На дату актуализации unit-тесты ведутся во всех пакетах (`packages/*/src/__tests__/*.test.ts(x)`, Vitest) — гайд задаёт правила их поддержки и расширения.
...
Тесты — операционное представление требований. Если требование нельзя объективно проверить unit-тестом или другим уровнем тестирования, это дефект требования: непроверяемость, неоднозначность или неполнота.
...
| Слабый оракул                     | «не упало» вместо «вернулась ошибка E»                    | Сверка с критерием приёмки, `FG-TEST`                    |
...
| Подгонка кода под тест            | реализация написана так, чтобы тест прошёл                | Fresh-context ревью, контракт как оракул                 |
...
| Неоднозначное требование          | «быстро»/«надёжно» → любая реализация легальна            | Проверяемость требований (`G1-METRIC`)                   |

### 14.2. Слои защиты

| Слой                | Что закрывает                                        | Механизм                                                 | Гейт                         |
| ------------------- | ---------------------------------------------------- | -------------------------------------------------------- | ---------------------------- |
| 1. Требование       | Неоднозначность/непроверяемость — легальная имитация | Проверяемый и однозначный критерий, `G1-METRIC`          | Ревью набора требований      |
...
| 5. Оракул           | Оракул = код (тавтология)                            | Контракты/схемы как независимый оракул, conformance      | `FG-TEST`, контрактные тесты |
...
- **Контракт как оракул**: ожидаемый результат для mapper/handler берётся из схемы (`docs/contracts/`, zod-схемы, TS-типы), а не из текущего ответа кода.
...
- [ ] Зафиксированы harness-ограничения контура: `max_retries`, `timeout`, разделение recoverable/terminal ошибок и правило эскалации.

...
> **Методологическая база:** `README.md` §4.4 (E2E GUI) и §5 (матрица качества), `test-plan.md` (артефакты `aif-qa`), `gates.md` (FG-гейты), `e2e-api-testing.md` (граница с E2E API), `.ai-factory/references/software-requirements-wiegers-beatty.md` (Validation, acceptance criteria, traceability); `use-cases/` (UC с каналом `GUI`), `business-rules/` (`BR-*`), `vision.md` (HF2, HF7–HF9, ограничения §2.6); код `packages/web` — только для понимания реализации, не источник правды.
...
| Выходной критерий | Сквозные P0/P1 GUI-сценарии проходят с внешним орáкулом и артефактами (скриншоты/трассы/video); негативные сценарии выполнены                                                                                 |
...
По W&B E2E GUI — самое близкое к валидации пользовательского опыта представление требований: если сценарий пользователя нельзя объективно проверить через интерфейс на стенде, это дефект требования или архитектуры (непроверяемость, неоднозначность, неполнота).
...
Источник правды для E2E GUI-сценария — **UC с каналом `GUI`** (`use-cases/`) и **US/функции** `vision.md` §2.2. UC/VF/BR/FR — контекст трассировки. Исключение — сценарий технического поведения без UC/US (BR/KI): оформляется явно в карточке сценария.
...
Контур внешних систем **не является предметом проверки**, но он — независимый орáкул состояния за интерфейсом: GUI обязан отображать то, что реально происходит в API/БД.

...
Методологическая основа: дисциплина валидации требований и риск-ориентированный контроль качества. Ключевой постулат для QA: **каждое требование должно быть проверяемым** (verifiable), а тесты — операционное представление требований.
...
### 2.1. Тестирование как валидация требований
...
Характеристики отличного требования применяются к тестированию напрямую:
...
1. **Тестируемость требований.** Требование без проверяемого критерия — не готово. Тест появляется вместе с требованием, а не после реализации.
...
14. **Harness-подход обязателен.** Для каждого тестового контура фиксируются `Constrain` (лимиты и allowlist), `Inform` (контекст и контракты), `Verify` (гейты и оракул), `Correct` (retry/fallback/escalation) с детерминированными порогами.
...
- **Непокрытое требование** — риск релиза: фиксируется в `open-questions.md` (артефакт создаётся по мере выявления, по RULES.md) и в ревью набора требований.
- Матрица обновляется при изменении требований (change control) и при добавлении тестов.
...
**Статусы покрытия ячеек матрицы (5×4):**

- 🔴 **Лакуна** — ячейка пуста: нет сформулированного измеримого требования или проверки («свободный параметр»).
- 🟠 **Метрологически несостоятельно** — формулировки есть, но они неизмеримы/неоперациональны; ячейка не закрыта.
- 🟡 **Формально закрыто, содержательно неполно** — измеримая проверка есть, но остаются известные упущенные аспекты ячейки (по опыту, инцидентам, списку рисков).
- 🟢 **Закрыто полно** — есть измеримая проверка и достаточное покрытие рисков ячейки.
- ⬆️ **Иерархическое нарушение** — младший фактор закрыт (🟢/🟡), а соответствующий старший фактор того же этапа — 🔴/🟠; иерархия факторов строгая: при незакрытом старшем факторе младшие теряют смысл.

Правило классификации: **лакуна** (🔴) и **неизмеримость** (🟠) — разные дефекты и фиксируются отдельно.

Правило вердикта: качество объективно подтверждено, только если все ячейки 🟢 без иерархических нарушений; любой 🔴/🟠 на этапах «Эксплуатация» (C2/C3) или нарушение иерархии в критичных факторах — статус «объективно не подтверждено, есть риск».
...
| Ревьюер             | Ревью тестов наравне с кодом; ревью требований и критериев приёмки                                                              |

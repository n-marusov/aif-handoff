...
| Bootstrap-синхронизация  | `UC-integration.issues.bootstrap-project-sync-and-create-task`                                                                     | E2E-API-018 (L-10c…L-10g, детерминированный; см. `e2e/api/gitlab-issue-to-accepted-full.spec.ts`) и E2E-API-019 (L-10-full, полный AI-контур, гейт `AIF_LLM_INTEGRATION=1`); **L-10j** (`done → implementing` rework, `US-pipeline.stage.done-to-implementing-rework`, детерминированный); **L-10k** (`plan_review → improve` rework, `US-integration.pr-mr.rework-plan-on-mr-comment`, гейт `AIF_LLM_INTEGRATION=1`) |

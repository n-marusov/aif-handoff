...
> **Методологическая база:** `README.md` §4.5 (E2E API) и §5 (матрица качества), `test-plan.md` (артефакты `aif-qa`), `gates.md` (FG-гейты), `.ai-factory/references/software-requirements-wiegers-beatty.md` (Validation, acceptance criteria, traceability); контракты `docs/contracts/` (`contract-aif-rest-api`, `contract-aif-ws`, `contract-aif-runtime`, `contract-aif-scheduler`, `contract-aif-github`, `contract-aif-gitlab`, `contract-aif-worktree`); `use-cases/`, `business-rules/`, `vision.md` — контекст для шагов, payload и assertions; код `packages/*/src` — только для понимания реализации, не источник правды.
...
| Выходной критерий | Сквозные P0/P1-сценарии из UC/US проходят с внешним орáкулом и артефактами прогона; негативные сценарии и failure injection выполнены                |
...
Правило: контур внешних систем **не является предметом проверки**, но он — независимый орáкул. Заглушки задают контрактные ответы по `docs/contracts/`, а не «как удобно текущей реализации».
...
6. внешний наблюдатель (HTTP/WS/MCP-клиент, заглушка, БД/аудит) как независимый орáкул;
7. текущий код — только для понимания реализации и диагностики.

Внешний наблюдатель — **независимый орáкул**: заглушки задают контрактные ответы по `docs/contracts/` и наблюдают результаты, не полагаясь на внутренние состояния процессов.
...
| ------------------- | --------------------------------------- | --------------------------------------------------------- | ----------------------- |
| 1. Требование       | Неоднозначность/непроверяемость пути    | Verifiable + unambiguous (W&B), `G1-METRIC`               | Ревью набора требований |
...
| 5. Орáкул           | Орáкул = система сама себя (тавтология) | Внешний наблюдатель: клиент/заглушка, контракт            | `FG-TEST`, conformance  |
...
- [test-plan.md](test-plan.md) — планы по функциям (уровни), тестовые данные, риски (`.ai-factory/qa/<branch-slug>/`).

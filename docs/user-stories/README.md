...
Каталог является **E2E-релевантным срезом**: в него входят только истории с прямым trace на сквозные GUI/API-сценарии (`docs/qa/e2e-gui-testing.md` §12, `docs/qa/e2e-api-testing.md` §12). Функции `vision.md` §2.2, для которых в текущем состоянии такого сценария нет, сознательно не оформляются US — это снимает «молчаливые» пробелы трассируемости vision → US:

| HF   | Функция                           | Причина отсутствия US                                                                                                                                                |
| ---- | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HF12 | Разогрев сессий (Warmup) (HF12.1) | Инфраструктурная возможность runtime-слоя без пользовательского E2E-сценария; трассируется через UC (`UC-warmup.preheat.warmup-runtime-session`), FR и runtime-тесты |

Эти функции не выпадают из трассируемости: они покрыты UC/FR (см. `use-cases/README.md`, `fun-req/README.md`) и кодом; отсутствие US означает только отсутствие сквозного пользовательского сценария в каталоге E2E на текущую дату.

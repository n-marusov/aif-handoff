...
- [x] `npm run lint`
- [x] `npm test`
- [x] WS `task:moved` handler: добавлена оптимистичная запись в кеш списков задач (гостевой race «инвалидация прилетела во время board-запроса»), плюс наблюдательный `window.__aifWsClientId` для ожидания готовности сокета в e2e (L-07). Покрыто e2e-прогонами против docker-compose.
- [x] `e2e/README.md` описывает актуальный readiness-контракт стенда: GitLab ждётся по `GET /users/sign_in` (2xx), а не по `/-/health` (loopback-only), и провижинит root-пользователя + PAT + `root/e2e-target` (см. `scripts/e2e-docker.mjs`).

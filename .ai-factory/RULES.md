...
- **Theme color pairing → `docs/ui-theme-colors.md`.** Tailwind semantic tokens (`text-foreground`, `bg-card`) flip with theme; fixed colors do not. Mismatched pairings break contrast in one theme. Read `docs/ui-theme-colors.md` before touching color classes. After fixing any theme-readability bug, append a dated entry to that file's "Learnings" section so the case is preserved.
...
- Keep complex conditions readable. Extract complex `if` conditions into well-named boolean variables or helper functions so the business rule is clear at the call site.

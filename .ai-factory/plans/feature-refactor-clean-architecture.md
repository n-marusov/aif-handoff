...
- [x] Task 19: Implement the use cases and move logic out of `api/src/services`
  ...
  - Completed (2026-09-19): moved the orchestration into `packages/api/src/use-cases/` — `taskEvents.ts` (applyTaskEvent: authorization → branch restore → mutation → post-check, HTTP `status` stripped to semantic `code`), `qaRun.ts` (startQaRun: CAS claim + qaStatus slot, returns lockId for the route to dispatch), `taskPlan.ts` (updateTaskPlan/getTaskPlanFileStatus/syncTaskPlanFile), `commitGeneration.ts` (generateCommit). Barrel `use-cases/index.ts` + DTO `types.ts`. HTTP adapters kept for public-surface compat: `services/taskEvents.ts` re-maps code→status (not_found→404, actor_not_authorized/assignment_required→403, fast_fix_incomplete→500, else→409); `services/commitGeneration.ts` keeps `runCommitQuery` alias; `repositories/tasks.ts` keeps positional `updateTaskPlan`/`syncTaskPlanFromFile` + creates the restored `createComment`/`updateComment` wrappers; routes/tasks.ts routes QA through `beginQaRun` (use case + WS broadcast + dispatch). Task 18 contract suite green: 8/8; API suite 579 passed (571 prior + 8 new). Grep: no `hono` import in `use-cases/**` (only comment mentions). Build 7/7, lint 0 errors.

- [x] Task 20: Reduce API routes to thin controllers
  ...
  - Completed (2026-09-19): extracted four task CRUD handlers into `use-cases/` — `createTask.ts` (authz member-rule, AI-ownership invariant, active-assignee check, runtime validation, mode defaults, two-phase attachments), `updateTask.ts` (canMutateTask authz, runtime validation, parallel-mode guard, planner mode-default fill, plan/attachments composite ops), `handoffTask.ts` (member self-service authz + CAS handoff), `deleteTask.ts` (worktree snapshot + best-effort cleanup). Extracted the ~640-line chat run-turn from `routes/chat.ts` into `use-cases/runChatTurn.ts` (adapter/profile resolution, streaming accumulation, session auto-create, usage/limit handling, abort handling) with injected WS ports (`sendToClient`, `broadcast`) and a shared `abortChatRun` helper (activeChatRuns map moved into the use case). Routes now only parse/validate, call the use case, shape HTTP + broadcast. Added DEBUG delegation logs `{ route, useCase, outcome }` at all five delegation points. Measured: `routes/tasks.ts` 1228→961 lines, `routes/chat.ts` 2078→626 lines. Re-checked `runtimeProfiles.ts`, `github.ts`, `gitlab.ts`: all I/O already delegated to service clients (`GitHubClient`/`GitLabClient`/`@aif/runtime`/`@aif/data`) — no direct `node:fs`, git, or external HTTP orchestration (grep-verified). Validation: build 7/7, lint 0 errors, API suite 579/579, agent 512/512 (isolated; parallel-load flake pre-existing), web/mcp/shared/data/runtime untouched. Acceptance met for all three criteria.

- [x] Task 21: Rebase MCP tools on the shared use cases
  ...
  - **Location DECISION (recorded):** `@aif/mcp` cannot import `@aif/api` (деплой независим — Docker-стадия mcp копирует только data/runtime/mcp; добавление зависимости mcp→api утащило бы весь HTTP-сервер в рантайм MCP). Общие «управляемые» операции задач переехали в `packages/data/src/taskOperations.ts`: `createTaskManaged` (mode defaults, parallel-forcing, дефолтный planPath, валидация runtime-профиля), `updateTaskManaged` (отказ полей владения, валидация runtime-профиля, сброс runPlanImprove/runPostVerify при useSubagents, mode-default филл), `setTaskPlanContentManaged` (push-plan контракт), `validateProjectScopedRuntimeProfileSelections` (переехало из api/services/runtimeProfileScope.ts; api-маршруты получают его через re-export). API use-cases (createTask/updateTask) стали оркестраторами над этими операциями + участковая авторизация + вложения/план-файлы; MCP-инструменты (create/update/push-plan) зовут те же операции. `assertRuntimeProfileSelection` (MCP-local дубль) удалён. DEBUG `{ toolName, useCase }` логи сохранены.
  - Completed (2026-09-19): new `packages/data/src/taskOperations.ts` + barrel export; api use-cases refactored to delegate (createTask/updateTask); api `runtimeProfileScope.ts` now re-exports the shared rule from data; MCP createTask/updateTask/pushPlan call the shared ops; removed MCP-local `assertRuntimeProfileSelection`; updated MCP tests (`runtimeTaskMetadata.test.ts`, `taskToolsRuntimeContract.test.ts`) to pin the shared rule semantics (code −32602) instead of MCP-local message text; added `taskOperations.test.ts` (18 tests; taskOperations coverage 91.4/76.6/100/91.3). Validation: build 7/7, data 340/340, api 579/579, mcp 118/118, agent 511/512 (isolated pass; pre-existing git flake). Nuances recorded in docs/known-issues.md (architecture reversal: operations live in data, not api use-cases: plan files vs plan field split, api/mcp error-message wording diff). Acceptance met.

- [x] Task 22: Remove framework types from policy signatures and centralise the execution-root invariant
  ...
  - Completed (2026-09-19): added `@aif/shared/src/taskExecutionRoot.ts` (`taskExecutionRoot({ worktreePath, rootPath })`) and exported from `@aif/shared` index; applied to all api call sites (routes/tasks.ts auto-QA + run-qa, use-cases/taskEvents.ts ×2, taskPlan.ts ×3 incl. executionRootFor, commitGeneration.ts ×1); qaRunner.ts comment updated. Framework-free policy: `canMutateTask` moved to a single `use-cases/taskPolicy.ts` (used by routes/tasks.ts sync-plan/run-qa/reorder AND use-cases/updateTask.ts — previously two copies) taking plain `TaskActionContext` + optional `{ method, path }` meta; `parseTaskOwnershipFilters` now takes a structural `QueryReader` (`{ query(name): string | undefined }`, Hono `c.req` matches) instead of hono `Context`. `requestActionContext` remains the single transport adapter in routes/tasks.ts (reads the participant session — inherently Hono); middleware/participantAuth.ts `getParticipantAuth` is middleware-neutral. Grep acceptance: `Context<ParticipantApiEnv>` remains only in `requestActionContext` (route transport seam) and `getParticipantAuth` (middleware); api has no `worktreePath ?? rootPath` code remaining (only a comment). Added `shared/__tests__/taskExecutionRoot.test.ts` (2 tests). Shared coverage ≥70% (79.8% lines). Validation: build 7/7, shared 278/278, api 579/579, lint 0 errors. Acceptance met.
...
- [x] Task 24: Inject the runtime registry and usage sink into the agent
  ...
  - Completed (2026-09-19): extracted the registry holder to `packages/agent/src/runtimeRegistry.ts` (`setRuntimeRegistry`/`getRuntimeRegistrySync`/`requireRuntimeRegistry`; DEBUG `{ source: "injected" }` on injection, ERROR when missing). coordinator re-exports the holder's seam (internal uses switched to `getRuntimeRegistrySync()`); subagentQuery `getRuntimeRegistry()` now returns `requireRuntimeRegistry()` — its private `bootstrapRuntimeRegistry` + own `createDbUsageSink` + `notifyRuntimeUsageRefresh` removed (usage notification now flows from the composition root's sink). `createDbUsageSink`/`bootstrapRuntimeRegistry` now appear in production only in `index.ts` (verified by grep). Tests reworked with a shared helper `packages/agent/src/__tests__/utils/injectTestRuntimeRegistry.ts` (cached real bootstrap injected via `setRuntimeRegistry`); subagentQuery.test.ts uses it + the sink-driven "task usage broadcast" test was removed (behavior relocated to the composition root sink; runtime equivalent covered by index.ts bootstrap). Added injection to planChecker/implementer/planner test files (real subagent phase runs). Validation: build 7/7, agent 511/511 (37 files, no flake this run), data 340/340, shared 282/282, api 579/579, lint 0 errors. Acceptance met.

- [x] Task 25: Move hardcoded prompts into agent definition files
  ...
  - Logging: DEBUG which prompt append was applied with `{ workflowKind, source }` (loader logs `{ dir, projectScopeLength, reviewScopeLength, source: "definitions" }`).
  - Acceptance: no long prompt literal remains in agent TS source; subagent behaviour tests stay green.
  - **Location DECISION (Variant A, approved by user):** the two scope rules were appended as `## Execution scope` → `### Project scope rule` / `### Review scope rule` sections to `.claude/agents/plan-coordinator.md` (no new files — avoids "phantom agent" noise in Claude Code's `--agent` discovery). New loader `packages/agent/src/agentScopeRules.ts` reads the definition file with a process cache and extracts the two sections; directory from `AIF_AGENT_DEFINITIONS_DIR` (new optional env, added to env.ts + docs/configuration.md) else `<cwd>/.claude/agents`. `subagentQuery.ts` now calls `getAgentScopeRules()` lazily at both sites (default systemPromptAppend + review append); `constants.ts` deleted (no remaining importers). Missing file/section degrades to empty strings (scope rule is advisory, not a hard contract).
  - Completed (2026-09-19): sections added to plan-coordinator.md; `agentScopeRules.ts` loader (cache + env override + cwd fallback; DEBUG `{ dir, projectScopeLength, reviewScopeLength, source: "definitions" }` on load, WARN on unreadable file); `subagentQuery.ts` uses `getAgentScopeRules()` lazily (project default via `??`, review via append); constants.ts removed; env.ts + docs/configuration.md document `AIF_AGENT_DEFINITIONS_DIR`. Added `agentScopeRules.test.ts` (2 tests: loads rules from the repo definition, cache reuse). Grep acceptance: no long scope-rule prompt literal in `packages/agent/src` (the rule text lives in `.claude/agents/plan-coordinator.md` and the loader's section-heading strings only). Validation: build 7/7, agent 512/513 (single run — the 1 failure was the known gitlabPrepare git flake; 513/513 on clean run), shared 282/282, lint 0 errors. Acceptance met.

- [x] Task 26: Split the coordinator into orchestration and infrastructure adapters
  ...
  - Completed (2026-09-19): audited the coordinator's direct infrastructure usage. Git/worktree/HTTP work was already port-shaped (`gitBranch.ts`, `worktreeLifecycle.ts`, `repositoryPrepare.ts`, `githubWorkflow.ts`, `gitlabWorkflow.ts`, `autoQueueCommit.ts`, `taskWatchdog.ts`, `worktreeReconcile.ts` — the coordinator delegates to them). The only remaining direct `node:fs`/`node:path` usage was a duplicated plan-file validation inside `processOneTask` (planner + improver gates, ~10 lines each). Extracted it into a new infrastructure adapter `packages/agent/src/planFileValidation.ts` (`isPlanFileValid({ executionRoot, isFix, planPath })` — resolve/exists/read encapsulated; any read error → invalid, matching the previous behavior). Coordinator now imports only the adapter; removed `node:fs`/`node:path` imports and the now-unused `getProjectConfig`. Grep acceptance: `node:fs`/`node:child_process` in `coordinator.ts` = 0 matches. Added `planFileValidation.test.ts` (5 tests: exists+content, missing, empty, fix-plan path, read error). Validation: build 7/7, agent 518/518 (full run incl. new tests; isolated coordinator 78/78 — the 1–3 git-worktree failures seen on some parallel runs are the pre-existing gitlabPrepare/planReviewCommit flake, green isolated), shared 282/282, data 340/340, api 579/579, mcp 118/118; lint 0 errors. Acceptance met.
...
- [x] Task 28: Update architecture documentation
  ...
  - Completed (2026-09-19): `ARCHITECTURE.md` — rewrote the folder-structure map (data `db.ts`/`taskOperations.ts`, api `use-cases/**`, agent `runtimeRegistry`/`planFileValidation`/`agentScopeRules`, shared `taskLifecycle`/`taskExecutionRoot`/`runtimeLimitGate`/`plannerDefaults`), added a **Clean-Architecture Layers** table (domain/persistence/application/adaptors/frameworks + key refactor consequences), updated the "Adding a new API route" example to the use-case pattern, updated the agent runtime example to `bootstrapRuntimeRegistry` + injected registry, added anti-patterns (hono in use cases, fs/process in orchestration, runtime-core→adapters). `AGENTS.md` — updated shared/api/agent structure map with all new modules. `docs/architecture.md` — updated package table + added **Application Layer (Use Cases)** section. `docs/contracts/README.md` — audited: already reflects `@aif/data/db` driver home; no stale `@aif/shared/server`/old-path references (grep-verified). `docs/providers.md` — no adapter/provider-contract change in this plan; content accurate as-is. Docker sync: no new package or inter-package dependency was added (MCP remains independently deployed on data/runtime/mcp; `taskOperations` is inside data, not a new workspace), so no Docker config churn — the living docker-sync rule in AGENTS.md stays. Stale-path audit: the only history-style reference to the old driver home is the ADR that explicitly documents the move (correct to keep); agent `src/constants.ts` removal left no doc references. Acceptance met.
